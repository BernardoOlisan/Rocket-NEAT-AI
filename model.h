...
public:
    ...
    std::vector<std::vector<float>> rocket_x_ = {{0}, {0}, {0}};
    std::vector<std::vector<float>> rocket_y = {{0}, {0}, {0}};
    ...
    // score and Genetic Encoding Scheme
    float score = 0.0;
    ...
    void rocket(int red, int green, int blue) {
        ...
        wall_west[0][0] = rocket_x * -1;

        rocket_x_[0][0] = rocket_x;
        rocket_y[0][0] = rocket_top - 0.05;
        ...
        point_distance[0][0] = std::sqrt(pow(x2-x1, 2) + pow(y2-y1, 2));
